...
import static com.xwlab.flightgaze.HttpUtils.sendJsonPost;
...
//        cursor.close();
     ...
     */
    public Passenger featureCmp(String fstr) {
        ...
        for (int i = 0; i < passengerList.size(); i++) {
            double sim = calculSimilar(feature, passengerList.get(i).getFaceFeature());
            if (sim > mSim) {
                index = i;
                mSim = sim;     //选出匹配度最高的
            }
        ...
        // 匹配戴口罩的特征
        for (int i = 0; i < passengerList.size(); i++) {
            double sim = calculSimilar(feature, passengerList.get(i).getFeatureWithMask());
            if (sim > mSim) {
                index = i;
                mSim = sim;     //选出匹配度最高的
            }
        ...
        if (mSim > 0.6) {
            passengerList.get(index).setMask(temp != mSim);
...
//                }
    ...
    private Runnable downloadRunnable = new Runnable() {
        ...
        public void run() {
            ...
            try {
                requestBody.put("service", "passenger.getListByFlight");
                requestBody.put("flight", FlightGazeApplication.getInstance().getFlight());
            ...
            try {
                ...
                FlightGazeApplication.getInstance().setFrom(from);
                FlightGazeApplication.getInstance().setTo(to);
                JSONArray passengers = result.optJSONArray("passengerList");
                for (int i = 0; i < passengers.length(); i++) {
                    JSONObject passengerJson = passengers.getJSONObject(i);
                    String name = passengerJson.optString("name");
                    String identifyId = passengerJson.optString("identifyId");
                    String flightClass = passengerJson.optString("class");
                    String seat = passengerJson.optString("seat");
                    int gate = passengerJson.optInt("gate");
                    String faceBase64 = passengerJson.optString("faceBase64");
                    String feature = passengerJson.optString("feature");
                    String featureWithMask = passengerJson.optString("featureWithMask");

                    DbHelper dbHelper = new DbHelper(FlightGazeApplication.getInstance(), "passenger", 1);
                    SQLiteDatabase db = dbHelper.getWritableDatabase();
                    ContentValues values = new ContentValues();
                    values.put("flight", FlightGazeApplication.getInstance().getFlight());
                    values.put("passenger_id", identifyId);
                    values.put("name", name);
                    values.put("face", faceBase64);
                    values.put("feature", feature);
                    values.put("feature_with_mask", featureWithMask);
                    values.put("city_from", from);
                    values.put("city_to", to);
                    values.put("flight_class", flightClass);
                    values.put("seat", seat);
                    values.put("gate", gate);
    ...
    private Runnable loadRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if(cursor.moveToFirst()) {
                FlightGazeApplication.getInstance().setFrom(cursor.getString(cursor.getColumnIndex("city_from")));
                FlightGazeApplication.getInstance().setTo(cursor.getString(cursor.getColumnIndex("city_to")));
                do {
                    String name = cursor.getString(cursor.getColumnIndex("name"));
                    String identifyId = cursor.getString(cursor.getColumnIndex("passenger_id"));
                    String flightClass = cursor.getString(cursor.getColumnIndex("flight_class"));
                    String seat = cursor.getString(cursor.getColumnIndex("seat"));
                    short gate = cursor.getShort(cursor.getColumnIndex("gate"));
                    String faceBase64 = cursor.getString(cursor.getColumnIndex("face"));
                    String feature = cursor.getString(cursor.getColumnIndex("feature"));
                    ...
                    Passenger passenger = new Passenger(identifyId, name, face, featureStringToArray(feature), featureStringToArray(featureWithMask), flightClass, seat, gate);
                    Logger.i(TAG, passenger.toString());
